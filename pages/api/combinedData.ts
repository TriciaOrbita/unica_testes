...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const mapToPessoaSaude = (item: DataItem): PessoaSaude => ({
      ...
      ultimaVisitaDomiciliarFormatada: item['ultima_visita_domiciliar_formatada'] as string,
      rn: Number(item['rn']),
    });
    ...
    if (limitedData.pessoasEscola.length < 1 && limitedData.pessoasCadastradas.length < 1 && limitedData.pessoasSaude.length < 1) {
      return res.status(404).json({ message: 'Nenhum dado encontrado.' });
    }

    // Retornar os dados combinados
    return res.status(200).json(limitedData);
  } catch (error: unknown) { // Tratar o erro como unknown

...
export const useSearchPerson = (searchTerm: string) => {
  ...
  const personFromPessoas = pessoas.find((p: Pessoa) => p.name.includes(searchTerm) || p.cpf === searchTerm);
  const personFromEscola = pessoasEscola.find((p: Pessoa) => p.name.includes(searchTerm) || p.cpf === searchTerm);
  const personFromSaude = pessoasSaude.find((p: Pessoa) => p.name.includes(searchTerm) || p.cpf === searchTerm);

  // Combinação dos dados, priorizando o dado disponível
  const personData: PersonData | null = {
    name: personFromPessoas?.name || personFromEscola?.name || personFromSaude?.name || '',
    cpf: personFromPessoas?.cpf || personFromEscola?.cpf || personFromSaude?.cpf || '',
    birthDate: personFromPessoas?.birthDate || personFromEscola?.birthDate || personFromSaude?.birthDate || '',
    age: personFromPessoas?.age || personFromEscola?.age || personFromSaude?.age || 0,
    address:
      personFromPessoas?.address ||
      (personFromEscola ? `${personFromEscola.local}, ${personFromEscola.rua} ${personFromEscola.numeroLogradouro}` : '') ||
      (personFromSaude ? `${personFromSaude.bairroDomicilio}, ${personFromSaude.logradouro}` : ''),
  };

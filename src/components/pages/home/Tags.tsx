...
const Tags = ({ healthData = [], cadunicoData = [], searchTerm }: TagsProps) => {
  ...
  const tags = [
    {
      ...
      condition: () =>
        (Array.isArray(healthData) && healthData.some(person => !person.nomeCidadao)) ||
        (Array.isArray(cadunicoData) && cadunicoData.some(person => !person.pessoaNome)),
    },
    {
      ...
      condition: () =>
        (Array.isArray(healthData) && healthData.some(person => person.unidadeSaude === 'Endereço Inválido')) ||
        (Array.isArray(cadunicoData) && cadunicoData.some(person => person.local === 'Endereço Inválido')),
    },
    {
      ...
      condition: () => {
        const isCpfSearch = searchTerm.length === 11 && /^[0-9]+$/.test(searchTerm); // Check if the search term is a CPF
        ...
        if (isCpfSearch) {
          ...
          return nomeCorrespondente !== undefined && vaccineConditions[nomeCorrespondente] !== undefined;
        } else {
          // For name searches, check if the searchTerm matches exactly with names in vaccineConditions
          return Array.isArray(healthData) && healthData.some(person => {
            // Only show if the person's name matches one in vaccineConditions
            return Object.keys(vaccineConditions).includes(person.nomeCidadao) && person.nomeCidadao.toLowerCase() === searchTerm.toLowerCase();
          });
        }
      },
    },
  ...
  const shouldRenderTags = typeof searchTerm === 'string' && searchTerm.trim() !== '';
  ...
  return (
    <div className="p-4 border border-gray-300 rounded-lg shadow-md w-full max-w-xs ml-auto">
      ...
      <div className="flex flex-col items-center space-y-2 w-full">
        {shouldRenderTags && tags.map(({ title, description, colorClass, condition }) => {

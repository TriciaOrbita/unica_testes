...
export function PersonData({ pessoasCadunico, pessoasSaude, searchTerm }: PersonDataProps) {
  // Filtra os dados do CadÚnico com base no termo de busca
  const filteredCadunicoData = pessoasCadunico.filter(pessoa =>
  ...
  const filteredHealthData = filteredCadunicoData.length === 0 ? safePessoasSaude.filter(pessoa => {
  ...
  const combinedData = filteredCadunicoData.length > 0 ? filteredCadunicoData : filteredHealthData;
  ...
  <AvatarFallback>
    ?
              ...
              <h2 className="font-medium">
                {combinedData[0].pessoaNome || combinedData[0].nomeCidadao || 'Nome do Cidadão'}
              </h2>
            </div>
          </div>
        )}
            ...
            <div className="mt-0 border-t border-neutral-100">
              <dl className="divide-y divide-neutral-100">
                {combinedData.map(pessoa => {
                  ...
                  return (
                    <div key={pessoa.coFatorCidadao || pessoa.pessoaNome} className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                      ...
                      <dd className="mt-1 text-sm/6 text-neutral-700 sm:col-span-2 sm:mt-0">
                        <p>{pessoa.pessoaCPF} {cpf}</p>
                      </dd>
                      <dt className="text-sm/6 font-medium text-neutral-900">Data de Nasc.</dt>
                      <dd className="mt-1 text-sm/6 text-neutral-700 sm:col-span-2 sm:mt-0">
                      ...
                      <dt className="text-sm/6 font-medium text-neutral-900">Endereço</dt>
                      <dd className="mt-1 text-sm/6 text-neutral-700 sm:col-span-2 sm:mt-0">
                        {pessoa.pessoaCPF ? (
